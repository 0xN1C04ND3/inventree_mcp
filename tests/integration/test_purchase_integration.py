...
@pytest.mark.asyncio
async def test_purchase_order_list_real_api():
    ...
    async with run_server_async(server.mcp) as url:
        async with Client(StreamableHttpTransport(url)) as client:
            result = await client.call_tool(
                "purchase_order", {"operation": "list", "limit": 10}
            )
            ...
            print(
                f"✓ Retrieved {len(data) if isinstance(data, list) else 'error'} purchase orders from InvenTree"
            )
